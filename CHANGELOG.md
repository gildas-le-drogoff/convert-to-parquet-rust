...
- **Compressed Files**: support for `.gz` and `.zst` input (automatic decompression to a temporary file)
...
- **Flexible Output**: `-o` can be a file or a folder (automatic detection)
...
- Verification of strict row ordering in the parallel pipeline
...
| -------------------------------- | ------------------------------------------------- |
| `csv_to_parquet file.csv`        | Unchanged, but auto-detection of other formats    |
| Error on `.json`                 | Automatic conversion to Parquet                   |
| `.gz` files unsupported          | Automatic decompression                           |
| Parquet → not possible           | `--to-csv` or `--to-jsonl`                        |
| Progress in rows/s               | Progress in MB/s                                  |
| Single Excel sheet → error       | All sheets → separate Parquet files               |
